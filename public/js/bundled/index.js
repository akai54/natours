...
/* DOM elements */ const mapBox = document.getElementById("map");
const loginForm = document.querySelector(".form--login");
const logOutBtn = document.querySelector(".nav__el-logout");
const userDataForm = document.querySelector(".form-user-data");
const userPasswordForm = document.querySelector(".form-user-password");
const bookBtn = document.getElementById("book-tour");
/* DELEGATION */ if (mapBox) {
    const locations = JSON.parse(document.getElementById("map").dataset.locations);
    (0, _mapbox.displayMap)(locations);
}
if (loginForm) loginForm.addEventListener("submit", (e)=>{
    e.preventDefault();
    /* INPUTS */ const emailValue = document.getElementById("email").value;
...
},{"./login":"7yHem","./mapbox":"3zDlz","./updateSettings":"l3cGY","./stripe":"10tSC"}],"7yHem":[function(require,module,exports) {
...
const login = async (email, password)=>{
    try {
        /* Try to authenticate using the given email and password */ const res = await (0, _axiosDefault.default)({
            method: "POST",
        ...
        /* If they are correct, then take the user to the homepage after 1,5 sec */ if (res.data.status === "success") {
...
const logout = async ()=>{
    try {
        ...
        /* Reload the page */ if (res.data.status === "success") location.reload(true);
    } catch (err) {
...
const displayMap = (locations)=>{
    mapboxgl.accessToken = (0, _keysDefault.default).mapboxKey;
    var map = new mapboxgl.Map({
        container: "map",
        style: "mapbox://styles/beshoux/cllcex03f00v501qp5np499pc",
        scrollZoom: false
    });
    /** This will send all the tours locations to the map and make it adapt it's size to show all of them at the same time */ const bounds = new mapboxgl.LngLatBounds();
    locations.forEach((loc)=>{
        /* Create marker */ const el = document.createElement("div");
        el.className = "marker";
        /* Add marker */ new mapboxgl.Marker({
            element: el,
            anchor: "bottom"
        }).setLngLat(loc.coordinates).addTo(map);
        /* Add popup */ new mapboxgl.Popup({
            offset: 30
        }).setLngLat(loc.coordinates).setHTML(`<p> Day ${loc.day}: ${loc.description}</p>`).addTo(map);
        /* Extend map bounds to include current location */ bounds.extend(loc.coordinates);
    });
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","../../keys":"AakGV"}],"AakGV":[function(require,module,exports) {
const keys = {
    stripeKey: "pk_test_51Nn35AIcvOjyyqlKFbLqJS9zNToQx0GuqbVKxIhKCSTh0IPgHYmpuhMmr09sXMGziXfhfJvbhbFr9ctbJJzXPeyY00ReY8qEWs",
    mapboxKey: "pk.eyJ1IjoiYmVzaG91eCIsImEiOiJjbGxiMnhuczQwMnUwM210Y3BseDl3YzF6In0.ZQ26bFUWVeY84k4M0rGiEQ"
};
module.exports = keys;
...
const updateSettings = async (data, type)=>{
    try {
        ...
        /* Try to update datam with the given name and email */ const res = await (0, _axiosDefault.default)({
            method: "PATCH",
            url,
            data,
            headers: {
...
var _keysDefault = parcelHelpers.interopDefault(_keys);
var _alerts = require("./alerts");
const stripe = Stripe((0, _keysDefault.default).stripeKey);
const bookTour = async (tourId)=>{
    try {
        /* 1) Get checkout session from API */ const session = await (0, _axiosDefault.default)(`/api/v1/bookings/checkout-session/${tourId}`);
        /* 2) Create checkout form + charge credit card */ await stripe.redirectToCheckout({
...
},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","../../keys":"AakGV"}]},["aC5kS","f2QDv"], "f2QDv", "parcelRequire11c7")
